...
def placeholder_inputs(num_point,num_channle):
...
def get_model(point_cloud, is_training,Transform=False, bn_decay=None):
    forloss = []
    idx=None
    if point_cloud.get_shape()[-1].value>3:
    ...
    if Transform:
        l0_xyz, tnet,l0_points= transform_moudule(l0_xyz,l0_points, is_training,idx, [64,128,1024],[[1, 1], [1, 1],[1, 1]], [512,256],num_channle=32, nsample=40, scope='Tnet1', bn_decay=bn_decay, bn=True, K=3)
    # PlaneNet layer
    l0_points, plane_feature ,_= plane_module(l0_xyz, l0_points,idx,centralize=True,num_channle=32, npoint=512,
                                          nsample=40, is_training=is_training, scope='Plane', pool='avg',use_xyz=True,bn=True, bn_decay=bn_decay,weight_decay=0.0)
    # Conv layers
    ...
    net = util.fully_connected(net, 40, activation_fn=None, scope='fc3')
